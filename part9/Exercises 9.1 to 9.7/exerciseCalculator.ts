...
const parseArguments = (args: string[]): ArgumentValues => {
	...
	const parsedTrainingHours: number[] = [];
...
const exerciseCalculator = (trainingHours: number[], targetHour: number) => {
	try {
		const result = calculateExercises(trainingHours, targetHour);
		return result;
	} catch (error: unknown) {
		let errorMessage = 'Something bad happened.';
		if (error instanceof Error) {
